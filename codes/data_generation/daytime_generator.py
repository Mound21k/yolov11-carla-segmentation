#!/usr/bin/env python3
"""
Daytime Dataset Generator for YOLOv11 Instance Segmentation
==========================================================

Generates comprehensive daytime driving scenarios with dynamic positioning
...
Usage: python daytime_generator.py [--samples 300] [--positions 5]
...
# Import the base generator (ensure it's in the same directory)
from carla_base_generator import EnhancedCARLAGenerator

logger = logging.getLogger(__name__)


class DaytimeDatasetGenerator(EnhancedCARLAGenerator):
    """Specialized generator for daytime driving scenarios."""
    
    def __init__(self, output_dir='datasets/daytime', samples_per_position=60, num_positions=5):
        super().__init__(output_dir=output_dir)
        
        self.condition_name = 'daytime'
        self.samples_per_position = samples_per_position
        self.num_positions = num_positions
        self.total_target_samples = samples_per_position * num_positions
        
        # Daytime-specific configuration
        self.daytime_config = {
            'enhanced_visibility': True,
            'traffic_density': 'high',
            'pedestrian_activity': 'high',
            'collection_speed': 'normal'
        }
    ...
    def setup_condition_directories(self):
        """Setup directories specific to daytime condition."""
        condition_dirs = [
        ...
        logger.info(f"Daytime dataset directories created")
    
    def generate_daytime_dataset(self):
        """Generate complete daytime dataset with dynamic positioning."""
        logger.info("="*60)
        logger.info("STARTING DAYTIME DATASET GENERATION")
        logger.info("="*60)
        ...
        try:
            ...
            for position_idx in range(self.num_positions):
                logger.info(f"\n--- POSITION {position_idx + 1}/{self.num_positions} ---")
            ...
            self.generate_daytime_report()
            
            logger.info("="*60)
            logger.info("DAYTIME DATASET GENERATION COMPLETED")
            logger.info("="*60)
            
            return True
            
        except Exception as e:
            logger.error(f"Error during daytime dataset generation: {e}")
            return False
    ...
    def collect_data_from_position(self, position_idx):
        ...
        try:
            ...
            logger.info(f"Position {position_idx + 1} completed: {position_stats['frames_collected']} frames")
            return True
    ...
    def collect_synchronized_frames(self, position_idx):
        ...
        while frames_collected < target_frames and (time.time() - start_time) < collection_timeout:
            ...
            # Check for synchronized frames with shorter timeout
            try:
    ...
    def process_and_save_frame(self, frame_data, position_idx, frame_idx):
        ...
        filename_base = f"daytime_pos{position_idx:02d}_frame{frame_idx:04d}_{timestamp}"
    ...
    def generate_daytime_report(self):
        """Generate comprehensive daytime dataset report."""
        ...
        report = {
            ...
            'collection_parameters': {
                ...
                'traffic_density': self.daytime_config['traffic_density']
            },
        ...
        # Print summary
        self.print_daytime_summary(report)
        
        logger.info(f"Daytime dataset report saved to: {report_path}")
    ...
    def print_daytime_summary(self, report):
        """Print comprehensive daytime dataset summary."""
        print("\n" + "="*70)
        print("DAYTIME DATASET GENERATION SUMMARY")
        print("="*70)
        
        dataset_info = report['dataset_info']
        print(f"Samples Collected: {dataset_info['total_samples_collected']}/{dataset_info['target_samples']}")
...
def main():
    """Main execution function for daytime dataset generation."""
    parser = argparse.ArgumentParser(description='Generate Daytime Dataset for YOLOv11')
    ...
    parser.add_argument('--output-dir', default='datasets/daytime', help='Output directory')
    parser.add_argument('--host', default='localhost', help='CARLA server host')
    parser.add_argument('--port', type=int, default=2000, help='CARLA server port')
    
    args = parser.parse_args()
    
    # Initialize daytime generator
    generator = DaytimeDatasetGenerator(
    ...
    try:
        success = generator.generate_daytime_dataset()
        
        if success:
            print(f"\n✅ Daytime dataset generation completed successfully!")
            print(f"📁 Dataset saved to: {generator.output_dir}")
            print(f"📊 Check the report file for detailed statistics")
        else:
            print("❌ Daytime dataset generation failed. Check logs for details.")
            return 1
