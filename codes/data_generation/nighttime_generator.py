#!/usr/bin/env python3
"""
Nighttime Dataset Generator for YOLOv11 Instance Segmentation
===========================================================

Generates challenging nighttime driving scenarios with dynamic positioning
and guaranteed coverage of all 4 target classes: car, bus, pedestrian, traffic_light

FIXES:
- Fixed file structure corruption
- Resolved RPC collision error
- Uses proven daytime logic with nighttime features

Usage: python nighttime_generator.py [--samples 30] [--positions 5]
...
class NighttimeDatasetGenerator(EnhancedCARLAGenerator):
    """Specialized generator for nighttime driving scenarios."""
    
    def __init__(self, output_dir='datasets/nighttime', samples_per_position=60, num_positions=5):
        super().__init__(output_dir=output_dir)
        
        self.condition_name = 'nighttime'
        self.samples_per_position = samples_per_position
        self.num_positions = num_positions
        self.total_target_samples = samples_per_position * num_positions
        
        # Nighttime-specific configuration
        self.nighttime_config = {
            'enhanced_lighting': True,
            'traffic_density': 'medium',
            'pedestrian_activity': 'low',
            'collection_speed': 'slower',
    ...
    def setup_condition_directories(self):
        """Setup directories specific to nighttime condition."""
        condition_dirs = [
        ...
        logger.info(f"Nighttime dataset directories created")
    
    def generate_nighttime_dataset(self):
        """Generate complete nighttime dataset with dynamic positioning."""
        logger.info("="*60)
        logger.info("STARTING NIGHTTIME DATASET GENERATION")
        logger.info("="*60)
        ...
        try:
            ...
            for position_idx in range(self.num_positions):
                logger.info(f"\n--- NIGHTTIME POSITION {position_idx + 1}/{self.num_positions} ---")
            ...
            self.generate_nighttime_report()
            
            logger.info("="*60)
            logger.info("NIGHTTIME DATASET GENERATION COMPLETED")
            logger.info("="*60)
            
            return True
            
        except Exception as e:
            logger.error(f"Error during nighttime dataset generation: {e}")
            return False
    ...
    def collect_data_from_position(self, position_idx):
        ...
        try:
            ...
            # Use base traffic spawning (proven to work)
            ...
            # Apply nighttime lighting to vehicles
            self.apply_nighttime_lighting()
            ...
            logger.info(f"Nighttime position {position_idx + 1} completed: {position_stats['frames_collected']} frames")
            return True
    ...
    def apply_nighttime_lighting(self):
        """Apply nighttime lighting to all spawned vehicles."""
        try:
            vehicle_count = 0
            for actor in self.actor_list:
                if hasattr(actor, 'type_id') and 'vehicle' in actor.type_id:
                    try:
                        # Enable nighttime lights
                        ...
                        # Occasionally enable high beams
                        ...
                        logger.debug(f"Failed to set lights for vehicle {actor.id}: {e}")
            
            logger.info(f"Applied nighttime lighting to {vehicle_count} vehicles")
            
        except Exception as e:
            logger.warning(f"Error applying nighttime lighting: {e}")
    ...
    def collect_synchronized_frames(self, position_idx):
        ...
        return {
            ...
            'average_fps': frames_collected / max(collection_time, 0.001)
        }
    ...
    def process_and_save_frame(self, frame_data, position_idx, frame_idx):
        ...
        filename_base = f"nighttime_pos{position_idx:02d}_frame{frame_idx:04d}_{timestamp}"
        ...
        metadata = {
            ...
            'weather_condition': 'nighttime',
            'lighting_conditions': 'low_light_with_artificial_lighting',
    ...
    def generate_nighttime_report(self):
        """Generate comprehensive nighttime dataset report."""
        # Safe total calculation
        total_samples = max(self.samples_collected, 1)
        ...
        report = {
            ...
            'collection_parameters': {
                ...
                'traffic_density': self.nighttime_config['traffic_density']
            },
            'quality_metrics': self.calculate_quality_metrics(),
            'nighttime_specific': {
                'vehicle_lighting_enabled': self.nighttime_config['vehicle_lights_enabled'],
        ...
        # Print summary
        self.print_nighttime_summary(report)
        
        logger.info(f"Nighttime dataset report saved to: {report_path}")
    
    def calculate_quality_metrics(self):
        """Calculate dataset quality metrics with safe division."""
        total_instances = sum(self.class_distribution.values())
        safe_samples = max(self.samples_collected, 1)
    ...
    def print_nighttime_summary(self, report):
        """Print comprehensive nighttime dataset summary."""
        print("\n" + "="*70)
        print("NIGHTTIME DATASET GENERATION SUMMARY")
        print("="*70)
        
        dataset_info = report['dataset_info']
        print(f"Nighttime Samples: {dataset_info['total_samples_collected']}/{dataset_info['target_samples']}")
        ...
        nighttime_specific = report['nighttime_specific']
        print(f"\nNighttime Features:")
        print(f"  Vehicle Lighting: {'Enabled' if nighttime_specific['vehicle_lighting_enabled'] else 'Disabled'}")
        print(f"  Reduced Pedestrians: {'Yes' if nighttime_specific['reduced_pedestrian_activity'] else 'No'}")
        print(f"  Artificial Lighting: {'Enhanced' if nighttime_specific['enhanced_artificial_lighting'] else 'Standard'}")
        
        print("\n" + "="*70)


def main():
    """Main execution function for nighttime dataset generation."""
    parser = argparse.ArgumentParser(description='Generate Nighttime Dataset for YOLOv11')
    ...
    parser.add_argument('--output-dir', default='datasets/nighttime', help='Output directory')
    parser.add_argument('--host', default='localhost', help='CARLA server host')
    parser.add_argument('--port', type=int, default=2000, help='CARLA server port')
    
    args = parser.parse_args()
    
    # Initialize nighttime generator
    generator = NighttimeDatasetGenerator(
    ...
    try:
        success = generator.generate_nighttime_dataset()
        
        if success:
            print(f"\n🌙 Nighttime dataset generation completed successfully!")
            print(f"📁 Dataset saved to: {generator.output_dir}")
            print(f"📊 Check the report file for detailed statistics")
        else:
            print("❌ Nighttime dataset generation failed. Check logs for details.")
            return 1
